/*
  # Fix validate_draft_can_start W2 warning wording

  ## Summary
  The previous W2 warning text said unresolved roster players are "excluded from
  the draft pool by default". This was inaccurate: unresolved players have no
  sports_player_id match so they cannot be assigned as keepers, but no
  draft_player_exclusions row is created for them and they are NOT blocked from
  the draft pool. Only ignored teams with player_pool_policy='unavailable' create
  actual exclusion rows.

  Updated W2 to accurately say "cannot be assigned as keepers until resolved"
  instead of implying they are blocked from the draft pool.
*/
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- W2. Unresolved roster players cannot be assigned as keepers.
    --     Note: these players are NOT blocked from the draft pool — they simply
    --     have no sports_player_id match and will be skipped during keeper selection.
    --     Use draft_player_exclusions (set via team mapping) to block players from
    --     the pool entirely.
    ...
    IF v_unresolved_count > 0 THEN
      v_warnings := v_warnings || format(
        '%s imported roster player%s could not be matched to a known player and '
        'cannot be assigned as keeper%s until resolved. '
        'They remain available in the draft pool.',
        v_unresolved_count,
