/*
  # Fix normalize_draft_scoring_rules ON CONFLICT syntax

  Postgres only supports ON CONFLICT ON CONSTRAINT for named table constraints
  (created with CONSTRAINT ... UNIQUE), not for partial unique indexes.
  For partial indexes, the correct syntax is:
    ON CONFLICT (columns) WHERE predicate DO UPDATE ...

  This recreates the function using that syntax.
*/
...
BEGIN
  ...
  LOOP
    ...
    LOOP
      ...
      -- For tiered rows use DO NOTHING (multiple ESPN statIds map to same key/threshold)
      -- For normal rows use the partial index predicate form of ON CONFLICT
      IF v_is_tiered THEN
      ...
      ELSE
        -- ON CONFLICT for partial index: must match index definition exactly
        -- Index: (draft_scoring_rule_id, stat_key) WHERE is_tiered = false
        INSERT INTO draft_scoring_rule_values AS dsrv (
          draft_scoring_rule_id, provider, stat_key,
          provider_stat_id, provider_stat_key,
          points_per_unit, threshold_min, threshold_max,
          is_tiered, raw_item
