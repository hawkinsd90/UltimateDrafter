/*
  # Add p_draft_scoring_rule_id parameter to get_draft_player_pool_with_rankings

  ## Change
  Adds an optional p_draft_scoring_rule_id uuid parameter (DEFAULT NULL).

  ## Join logic
  - When p_draft_scoring_rule_id IS NULL (all existing sources: sleeper, espn, fantasypros):
    JOIN requires pr.draft_scoring_rule_id IS NULL (global rankings)
  - When p_draft_scoring_rule_id IS NOT NULL (Last Season custom rows):
    JOIN requires pr.draft_scoring_rule_id = p_draft_scoring_rule_id (league-specific)

  This is the only change from the previous version. All parameters, return columns,
  sort behavior, and security are identical.
*/
...
AS $$
  ...
  LEFT JOIN player_rankings pr
    ...
    AND (
      -- When no custom rule is requested, match global (non-custom) rankings
      (p_draft_scoring_rule_id IS NULL     AND pr.draft_scoring_rule_id IS NULL)
      OR
      -- When a custom rule is requested, match only that rule's rows
